...
async function register(dir: string) {
	...
	for (const file of files) {
		if (file.endsWith(".js") || file.endsWith(".ts")) {
			...
			if (!thing) continue;
			let thing2;
			if (dir.endsWith("commands") || dir.endsWith("slashCommands") || dir.endsWith("contextMenus")) {
				let name = thing.name;
				const subCommand = thing.isSubCommand && thing.subCommand;
				if (subCommand) {
					name = `${thing.name}-${subCommand}`;
				}
				thing2 = name;
			}
