...
export default {
    ...
    function: async function (interaction: Interaction) {
        ...
        const cdKey = `${interaction.customId}-${interaction.user.id}-${interaction?.guildId}`;

        if (!selectMenu) return;
        const existingCooldown = client.cooldowns.get(cdKey) as Cooldown 
        if (existingCooldown) {
        ...
        if (selectMenu.cooldown !== 0 && (interaction.guild && memberPerms.has(PermissionsBitField.Flags.Administrator))) {
            ...
            client.cooldowns.set(cdKey, { user: interaction.user.id, id: selectMenu.id, until });
            setTimeout(() => {
