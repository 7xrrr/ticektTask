...
export default {
	...
	function: async function (interaction: CommandInteraction) {
		...
		const cmd = subCommand ? `${interaction.commandName}-${subCommand}` : interaction.commandName;
		const cdKey = `${cmd}-${interaction.user.id}-${interaction?.guildId ? interaction.guildId : "dm"}`;
		...
		if (command.dmCommand) {
			...
			const existingCooldown = client.cooldowns.get(cdKey) as any;
			if (existingCooldown) {
			...
			const existingCooldown = client.cooldowns.get(cdKey);
			if (existingCooldown) {
				const now = Math.floor(Date.now() / 1000);
				if (now < existingCooldown.until) {
					const cooldownEmbed = new EmbedBuilder()
		...
		if (command.cooldown) {
			...
			client.cooldowns.set(cdKey, ({ user: interaction?.user.id, command: command.name, until }));
			setTimeout(() => {
